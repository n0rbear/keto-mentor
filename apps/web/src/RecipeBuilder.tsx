...
type Recipe = { id: string; userId: string; title: string; description?: string; instructions?: string[]; servings?: number; finishedWeightGrams?: number; visibility: "private" | "public"; user: { id: string; username: string }; ingredients: Ingredient[]; nutrition: { total: NutritionSlice; perServing: NutritionSlice | null; per100g: NutritionSlice | null } };
type ImportIngredient = { originalText: string; omitted?: boolean; parsedQuantity?: number; parsedUnit?: string; parsedFoodQuery: string; preparation?: string; resolution: string; selectedFood: Food | null; candidates: Food[]; quantity: { status: string; grams?: number; requiresConfirmation: boolean } | null; canConfirm: boolean };
...
export function RecipeBuilder({ lang, state, currentUserId, onMealAdded }: { lang: Lang; state: ApiState; currentUserId: string; onMealAdded: () => Promise<void> }) {
  ...
  function startNew() { setEditing(null); setTitle(""); setDescription(""); setInstructions([]); setServings(""); setFinishedWeight(""); setVisibility("private"); setIngredients([]); setStatus(""); setImportUrl(""); setImportPreview(null); setReviewIndex(null); setTab("edit"); }
  function startEdit(recipe: Recipe) { setEditing(recipe); setTitle(recipe.title); setDescription(recipe.description ?? ""); setInstructions(recipe.instructions ?? []); setServings(recipe.servings?.toString() ?? ""); setFinishedWeight(recipe.finishedWeightGrams?.toString() ?? ""); setVisibility(recipe.visibility); setIngredients(recipe.ingredients); setStatus(""); setImportUrl(""); setImportPreview(null); setReviewIndex(null); setTab("edit"); }
  ...
  return <section className="recipe-shell card">
    ...
    {tab === "edit" ? <div className="recipe-editor">
      ...
      {importPreview && <section className="rounded-xl border border-borderSoft p-4" aria-label={importText[lang].preview}><h3>{importPreview.title}</h3>{importPreview.servings && <p>{importPreview.servings} {lang === "de" ? "Portionen" : lang === "en" ? "servings" : "adag"}</p>}<ul className="recipe-detail-list">{importPreview.ingredients.map((item, index) => { const valid = !item.omitted && item.canConfirm && item.selectedFood && item.quantity?.status === "resolved" && item.quantity.grams; return <li key={`${item.originalText}-${index}`}><strong>{item.originalText}</strong> — {item.omitted ? "Kihagyva" : valid ? `${item.selectedFood!.name}, ${Math.round(item.quantity!.grams! * 10) / 10} g · ${importText[lang].resolved}` : item.selectedFood ? importText[lang].review : importText[lang].unresolved}{!item.omitted && !valid && <button type="button" className="btn secondary ml-2" onClick={() => { setReviewIndex(index); setCandidate(item.selectedFood); setCandidateGrams(item.quantity?.grams ? String(item.quantity.grams) : ""); }}>{importText[lang].resolve}</button>}<button type="button" className="btn secondary ml-2" onClick={() => setImportPreview((preview) => preview ? { ...preview, ingredients: preview.ingredients.map((row, rowIndex) => rowIndex === index ? { ...row, omitted: !row.omitted } : row) } : preview)}>{item.omitted ? "Visszaállítás" : "Kihagyás"}</button></li>; })}</ul>{instructions.length > 0 && <ol className="recipe-detail-list">{instructions.map((instruction, index) => <li key={index}>{instruction}</li>)}</ol>}{importBlocked && <p className="text-sm text-muted" role="alert">{importText[lang].blocked}</p>}</section>}
      <div className="grid gap-3 md:grid-cols-2"><label>Recept neve<input className="field" value={title} onChange={(e) => setTitle(e.target.value)} maxLength={120}/></label><label>Leírás (opcionális)<input className="field" value={description} onChange={(e) => setDescription(e.target.value)} maxLength={2000}/></label><label>Elkészítési lépések<textarea className="field" value={instructions.join("\n")} onChange={(e) => setInstructions(e.target.value.split(/\r?\n/).map((line) => line.trim()).filter(Boolean).slice(0, 100))}/></label><label>Adagok<input className="field" type="number" min="0.1" step="0.1" value={servings} onChange={(e) => setServings(e.target.value)}/></label><label>Kész tömeg (g)<input className="field" type="number" min="0.1" step="0.1" value={finishedWeight} onChange={(e) => setFinishedWeight(e.target.value)}/></label></div>
