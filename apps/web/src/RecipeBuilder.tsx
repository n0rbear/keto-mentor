...
type Ingredient = { id?: string; foodId: string; quantityGrams: number; preparation?: string; sortOrder?: number; food: Food };
...
export function RecipeBuilder({ lang, state, currentUserId, onMealAdded }: { lang: Lang; state: ApiState; currentUserId: string; onMealAdded: () => Promise<void> }) {
  ...
  function startNew() { setEditing(null); setTitle(""); setDescription(""); setServings(""); setFinishedWeight(""); setVisibility("private"); setIngredients([]); setStatus(""); setTab("edit"); }
  function startEdit(recipe: Recipe) { setEditing(recipe); setTitle(recipe.title); setDescription(recipe.description ?? ""); setServings(recipe.servings?.toString() ?? ""); setFinishedWeight(recipe.finishedWeightGrams?.toString() ?? ""); setVisibility(recipe.visibility); setIngredients(recipe.ingredients); setStatus(""); setTab("edit"); }
  function addIngredient() {
    ...
    setIngredients((items) => [...items, { foodId: candidate.id, quantityGrams: grams, food: candidate, sortOrder: items.length }]); setCandidate(null); setCandidateGrams("100"); setResetVersion((value) => value + 1); setStatus("");
  }
  ...
  return <section className="recipe-shell card">
    ...
    {tab === "edit" ? <div className="recipe-editor">
      ...
      <div className="recipe-actions"><button className="btn secondary" onClick={() => setTab("mine")}>Mégse</button><button className="btn primary" onClick={() => saveRecipe().catch(() => setStatus("A recept mentése nem sikerült."))}><Save size={17}/>Mentés</button></div>
