...
function eventText(event: DiagnosticEvent, lang: Lang): string {
  ...
  switch (event.code) {
    ...
    case "preview_match": return withLabel(hu ? "Valószínű találat, de nem eléggé biztos — ellenőrzés szükséges." : de ? "Wahrscheinlicher Treffer, aber nicht sicher genug — Prüfung nötig." : "A likely match, but not confident enough — review needed.", event);
    case "ambiguous": return withLabel(hu ? "Több, hasonlóan jó találat is van — válassz egyet." : de ? "Mehrere ähnlich gute Treffer — bitte auswählen." : "Multiple similarly-strong matches — a choice is needed.", event);
    ...
    case "confirmation_required": return withLabel(hu ? "Ehhez az ételhez megerősítés szükséges." : de ? "Für dieses Lebensmittel ist eine Bestätigung nötig." : "This food needs confirmation.", event);
