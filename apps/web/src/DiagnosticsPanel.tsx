...
function eventText(event: DiagnosticEvent, lang: Lang): string {
  ...
  switch (event.code) {
    ...
    case "local_match": return p.source === "reference"
      ? (hu ? `Megtaláltuk a Keto Mentor ételadatbázisában: "${p.title}" — webes keresés nélkül.` : de ? `In der Keto-Mentor-Gerichtedatenbank gefunden: "${p.title}" — ohne Websuche.` : `Found in the Keto Mentor dish database: "${p.title}" — no web search needed.`)
      : (hu ? `Megtaláltuk a saját receptjeid között: "${p.title}".` : de ? `In deinen eigenen Rezepten gefunden: "${p.title}".` : `Found among your own recipes: "${p.title}".`);
