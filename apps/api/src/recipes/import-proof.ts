...
export function createRecipeImportProof(userId: string, sourceUrl: string, secret = defaultSecret(), now = Date.now()) {
  if (!isValidUserId(userId) || !isSafeSourceUrl(sourceUrl) || !Number.isInteger(now)) throw invalidProof();
...
export function verifyRecipeImportProof(proof: string, userId: string, sourceUrl: string, secret = defaultSecret(), now = Date.now()) {
  ...
  if (!isImportProofPayload(payload) || payload.userId !== userId || payload.sourceUrl !== sourceUrl || payload.exp <= now) throw invalidProof();
...
function isImportProofPayload(value: unknown): value is ImportProofPayload {
  ...
  return Object.keys(payload).every((key) => ["v", "userId", "sourceUrl", "method", "exp"].includes(key))
    ...
    && payload.method === "schema_org_json_ld"
