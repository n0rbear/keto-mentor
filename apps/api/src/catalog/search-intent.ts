...
canonicalConcept and searchTerms must name the FOOD ITSELF at its most generic level — never a specific prepared dish, product, brand, or category that merely contains, uses, or is flavored by that food, UNLESS the input phrase itself already names that category (see the next rule). Correct: "burgonya" -> "potato" (NOT "bread potato", NOT "potato bread", NOT "potato chips", NOT "potato soup"). Correct: "só" -> "salt" (NOT "butter salted", NOT "salted crackers", NOT "salted pork"). Correct: "sertészsír" -> "lard" or "pork fat" (NOT "bologna beef and pork", NOT "pork sausage"). Correct: "tejföl" -> "sour cream" (NOT "sour cream cake"). Correct: "Erdapfel" -> "potato". Correct: "aubergine" -> "eggplant". Correct: "minced beef" -> "ground beef".
The rule above never licenses REMOVING a product-category word the input phrase itself already states (sauce, dressing, spread, dip, soup, juice, ...). "tejfölös szósz" / "sour cream sauce" -> canonicalConcept/searchTerms must stay "sour cream sauce" (or an equivalent sauce term), NEVER simplified down to bare "sour cream" — a sauce made with sour cream is not the same food as sour cream itself. Never move that category word into the "preparation" field either; "preparation" is only for a real doneness/cut/state distinction (raw/cooked/cured/smoked), never for "this is a sauce/dressing/spread" category information — that belongs in canonicalConcept/searchTerms as stated.
