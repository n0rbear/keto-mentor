...
import { dishCoreKey } from "./local-recipe-lookup.js";
...
export type UnifiedFoodItem = { type: "food"; food: any; via: "name" | "meaning" };
export type UnifiedSearchResult =
  | { kind: "barcode"; barcode: string }
  | { kind: "results"; query: string; items: Array<UnifiedRecipeItem | UnifiedFoodItem>; meaning: { tried: boolean; terms: string[] } };
...
export async function unifiedSearch(prisma: PrismaClient, rawQuery: string, meaning: boolean, deps: UnifiedSearchDeps): Promise<UnifiedSearchResult> {
  ...
  const [recipes, foods] = await Promise.all([findRecipes(prisma, query, normalized, deps.userId), searchFoods(prisma, query, MAX_FOODS) as Promise<any[]>]);
  ...
  if (meaning && deps.searchIntentProvider && deps.searchIntentProvider.id !== "disabled" && !foods.some((food) => food.match && isTrustedLocalMatch(food.match))) {
    if (!deps.meaningLimiter || deps.meaningLimiter.consume(deps.userId)) {
      tried = true;
      const intent = await deps.searchIntentProvider.generate({ foodQuery: query, foodLocale: deps.foodLocale }).catch(() => null);
  ...
  return { kind: "results", query, items, meaning: { tried, terms } };
}
...
async function findRecipes(prisma: PrismaClient, query: string, normalized: string, userId: string): Promise<UnifiedRecipeItem[]> {
  ...
  const ownHits = own
    .filter((row) => { const title = normalizeSearch(row.title); return title.includes(normalized) || dishCoreKey(row.title).includes(core); })
    .slice(0, MAX_RECIPES)
  ...
  for (const [alias, ids] of Object.entries(REFERENCE_DATA.aliases)) {
    if (!alias.includes(normalized) && !alias.includes(core)) continue;
