...
# ---------------------------------------------------------------------------
# 5. App data (apps/api/src/reference-dishes/hu-pilot.data.ts).
# Every variant is exactly ONE standard serving: each part's raw ingredients
# are scaled by (serving grams / part finished weight). Water carries mass
# only (finished weight), so it is not a recipe ingredient.
# ---------------------------------------------------------------------------
SIDE_WITH = {
...
def build_variants(parts, dishes):
    ...
    for d in dishes:
        ...
        for side, plist in options:
            ...
            titles = dict(d["names"])
            if side:
                titles = {loc: f"{titles[loc]} {SIDE_WITH[side][loc]}" for loc in ("hu", "de", "en")}
            grams = {}
            roles = {}
            ...
            variants.append({
                ...
                "sources": sorted({u for pid, _ in plist for u in parts[pid]["sources"]}),
            })
            ids.append(vid)
            for loc in ("hu", "de", "en"):
                add_alias(titles[loc], [vid])
...
def emit_ts(variants, aliases, path):
    import hashlib
    food_keys = {k: {"catalog": ({"source": v[3].split(":")[0], "sourceId": v[3].split(":")[1]} if v[3] and ":" in v[3] else None), "hu": v[0]} for k, v in FOOD_KEYS.items() if k != "water"}
    payload = {"foodKeys": food_keys, "variants": variants, "aliases": aliases}
    ...
    path.write_text(
        ...
        f"export const REFERENCE_DATA_VERSION = \"{version}\";\n\n"
        f"export const REFERENCE_DATA: ReferenceDishData = {body};\n",
        encoding="utf-8")
    return version
...
def main():
    ...
    emit_ts(variants, aliases, OUT.parent.parent / "apps" / "api" / "src" / "reference-dishes" / "hu-pilot.data.ts")
